...
# 
# ggplot(tidy_set, aes(x = key, y = fct_rev(id), fill = missing)) +
...
```{r}
# x <- missing_data.frame(drug_study)
# image(x)
```
...
ggplot(data = opioid_users, mapping = aes(ANLYRTOT)) + 
  geom_histogram(binwidth = 10, fill="#663399") + 
...
The histogram above is unimodal showing that most opioid misusers have taken the opioid less than ten times a year. To be consistent with many previous opioid investigations, we define frequent users as those with 100 or more days of nonmedical opioid use in the past year and highly frequent as those with 200 or more days of nonmedical use. In our sample, only 2.6% of users can be classified as highly frequent users, whereas 67.3% said they used prescription painkillers non-medically on a frequent basis and the remaining 30.1% reporting infrequent use.  
# hmm the frequency column in the dataframe supports this but doesnt this seem completely false based on how the histogram looks?? im confused...
...
Finally, we look at the relationship between income and crime, specifically focusing on arrest rates for non-medical opioid users and comparing this result to non-users. 
...
nonusers_plot <- ggplot(drug_study[!is.na(drug_study$Booked) & drug_study$Opioid=="no",]) +
  aes(x = Income, fill = Booked) +
...
Overall, just about 1/3 of opioid users have been arrested and booked for a crime. Broken down by income bracket, we can see that as income increases, the proportion of users who have been arrested decreases. For the poorest group, 39.7% of users have been arrested, and for the wealthiest group, 27.4% of users have been arrested. For non-users, the story is different in some ways and similar in others - overall, the proportion of non-users who have been arrested is 11%, which is much lower than what we observed overall for users. We still see that as income increases, the proportion of non-users who have been arrested decreases. For the poorest group, 15% of users have been arrested, and for the wealthiest group, only 7.5% of users have been arrested. Crime proves to have an interesting relationship with opioid use, so we now explore it further by breaking down arrests across various opioid usage frequencies.

```{r}
p = ggplot(data = opioid_users[!is.na(opioid_users$Booked),], mapping = aes(Booked)) + 
  geom_bar(aes(y = ..prop.., group = 1), fill = c("#F8766D", "#00BFC4", "#F8766D", "#00BFC4", "#F8766D", "#00BFC4")) + 
  facet_wrap(~frequency) +
  labs(title = "Ever Been Arrested and Booked for Breaking Law", x = "Been Booked", y = "Proportion")  
show(p)
```
For both infrequent and frequent opioid users, we see similar approximate ratios of about 70% of users having been booked and 30% never having been booked. However, for highly frequent users, we see a big spike in the number of arrests in that about half of all users have been booked before and half have not. This finding is not especially shocking, but it interesting to see such a sharp disparity between frequent and highly frequent opioid users in arrest rates and nearly no difference in arrest rates between infrequent and frequent users. This may lead us to believe that highly frequent users may have particularly serious issues in their lives that lead to both high opioid abuse and stronger likelihood of committing crimes. Mental health is one potential cause that we choose to investigate.


### Physical and Mental Health's Relationship to Opioid Abuse

A great deal of opioid abusers claim that they use opioids to self-medicate - for chronic pain, anxiety, depression, insomnia, or another condition. Luckily, the NSDUH survey also includes questions about various health conditions that we can explore in relation to opioid use. A lot of the data is for specific illnesses such as Tuberculosis and Asthma, which don't have strong ties to painkiller use, so instead we specifically look at categories that may be more tied to misuse - anxiety and depression.
...
p1 <- ggplot(non_users[!is.na(non_users$Anxiety),], mapping = aes(Anxiety)) +
  geom_bar(aes(y = ..prop.., group = 1), fill = c("#F8766D", "#00BFC4")) +
...
m1 <-  ggplot(data = opioid_users) +
   geom_mosaic(aes(x = product(Anxiety, Sex), fill=Anxiety), na.rm=TRUE, show.legend = FALSE) +
   labs(x="Sex", y="Has Anxiety", title='Gender Breakdown of Opioid Users with Anxiety')

m2 <-  ggplot(data = opioid_users) +
   geom_mosaic(aes(x = product(Depression, Sex), fill=Depression), na.rm=TRUE, show.legend = FALSE) +
   labs(x="Sex", y="Has Depression", title='Gender Breakdown of Opioid Users with Depression')

grid.arrange(m1, m2, ncol=2)

p1 <-  ggplot(data = opioid_users) +
...
s2 <-  ggplot(data = opioid_users) +
   geom_mosaic(aes(x = product(Depression, Age), fill=Depression), na.rm=TRUE, show.legend = FALSE) +
   labs(x="Age", y="Has Depression", title='Age Breakdown of Opioid Users with Depression')

grid.arrange(s1, s2, ncol=2)
```

# GENDER NOT THAT INTERESTING? 
The mosaic plots above provide some interesting depth to our exploration of opioid users dealing with anxiety and depression. We see very clearly that women are nearly twice as likely as men to experience anxiety and depression, which is a well-studied topic not necessarily tied to opioid use. The Anxiety and Depression Association of America assets that women are twice as likely to have anxiety as men, and the most common disorder to co-occur with Anxiety is depression. 

# FREQ OF USE P INTERESTING? 
The frequency of use breakdown shows some curious results. Highly frequent opioid users are the most likely to have anxiety and depression. There could be a variety of reasons for this finding such as taking opioids regularly may be used to "cope" with these disorders. More likely, however, is a hidden underlying variable such as chronic illness that may be correlate to anxiety and depression as well as highly frequent opioid use. The Harvard Women's Health Watch found that people with chronic respiratory disorders have a higher likelihood of developing anxiety disorders, and worryingly also asserted that "anxiety is a cause for non-compliance with medical treatment." This could either mean not regularly keeping up with medications and self-care or more relevant to our analysis, abusing prescribed painkiller medications. Unfortunately, the NSDUH data does not provide very much data on chronic conditions, so these interesting questions could only be further researched with an adjacent data set.  

This frequency of use mosaic plot also displays that infrequent users are more likely to have anxiety and depression as compared to frequent users. This seems to contradict the idea of using painkillers as a regular "coping" mechanism for mental health disorders and is therefore a relatively unexpected observation. Again, we must consider that underlying variables may have these visible results.  

# AGE KINDA INTERESTING?
Lastly, we explore anxiety and depression rates in opioid users broken down by age groups. As we transition from younger to older users, we see a general trend of increasing likelihood of anxiety and depression, meaning the younger opioid users have the lowest proportion of anxiety and depression while the older opioid users have the highest. The Anxiety and Depression Associate of America reaffirms these results by providing statistics that one in eight (12.5%) children are affected by anxiety disorders, which often co-occur with depression, where as 18.1% of the adult population in the US are prone to anxiety disorders. 

This leads to the takeaway that youth opioid misuse is less tied to anxiety and depression than adult misuse. As a result, there must be different causes for youth misuse such as social environment, extracurriculars, peer pressure, and more. We go on to explore the youth demographic in more depth in the next section.
