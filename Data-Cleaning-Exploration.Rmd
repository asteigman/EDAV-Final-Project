...
freq_opioid_users <-  opioid_users[opioid_users$ANLYRTOT >= 100,]
highly_freq_opioid_users <-  freq_opioid_users[freq_opioid_users$ANLYRTOT >= 200,]
...
summary(opioid_users$CATAG7)
summary(freq_opioid_users$CATAG7)
summary(highly_freq_opioid_users$CATAG7)

age_breakdown <- as.data.frame(summary(opioid_users$CATAG7))
age_breakdown <- cbind(age_breakdown, as.data.frame(summary(freq_opioid_users$CATAG7))[-8,])
age_breakdown <- cbind(age_breakdown, as.data.frame(summary(highly_freq_opioid_users$CATAG7))[-8,])
colnames(age_breakdown)[1] <- "Opioid Users"
colnames(age_breakdown)[2] <- "Frequent Opioid Users"
colnames(age_breakdown)[3] <- "Highly Frequent Opioid Users"

age_breakdown
ggplot(age_breakdown, aes(fill=condition, y=value, x=specie)) + 
...
# We see a pretty believable / expected trend. Users who do opioids frequently and highly frequently seem to be poorer - with salaries under $50000. It's interesting that across all types of users, the least popular income bracket for users is $50-75k.  When looking at all opioid users, we see a nearly equal number of men and women, but as we examine the frequent and the highly frequent users, a larger percentage of the users become male.  
